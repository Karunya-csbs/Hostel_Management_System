...
function dispatchFunction($functionName, $params = []) {
    ...
    switch ($functionName) {
        ...
        case 'getPendingLeaveCount':
            ...
            if ($genderScope !== null) {
                $safeGender = mysqli_real_escape_string($conn, $genderScope);
                $sql .= " AND la.Reg_No IN (SELECT s.roll_number FROM students s LEFT JOIN rooms r ON s.room_id = r.room_id LEFT JOIN hostels h ON r.hostel_id = h.hostel_id WHERE h.gender = '" . $safeGender . "')";
            }
        ...
        case 'getBlockedStudentsCount':
            ...
            $sql = "SELECT COUNT(DISTINCT bs.student_id) as count FROM blocked_students bs";
            
            // Add gender scope filter for male_admin and female_admin
            if ($genderScope !== null) {
                $safeGender = mysqli_real_escape_string($conn, $genderScope);
                $sql .= " INNER JOIN students s ON bs.student_id = s.student_id LEFT JOIN rooms r ON s.room_id = r.room_id LEFT JOIN hostels h ON r.hostel_id = h.hostel_id WHERE h.gender = '" . $safeGender . "'";
            } else {
                $sql .= " WHERE 1=1";
            }
        ...
        case 'getTotalStudentsCount':
            ...
            if ($genderScope !== null) {
                $safeGender = mysqli_real_escape_string($conn, $genderScope);
                $sql .= " LEFT JOIN rooms r ON s.room_id = r.room_id LEFT JOIN hostels h ON r.hostel_id = h.hostel_id WHERE s.status='1' AND h.gender = '" . $safeGender . "'";
            } elseif ($hostelId) {
